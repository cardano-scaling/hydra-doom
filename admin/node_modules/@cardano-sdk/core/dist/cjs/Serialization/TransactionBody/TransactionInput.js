...
class TransactionInput {
    ...
    static fromCbor(cbor) {
        ...
        if (length !== TRANSACTION_INPUT_ARRAY_SIZE)
            throw new util_1.InvalidArgumentError('cbor', `Expected an array of ${TRANSACTION_INPUT_ARRAY_SIZE} elements, but got an array of ${length} elements`);
        const txId = reader.readByteString();
        const index = reader.readInt();
        const input = new TransactionInput(util_1.HexBlob.fromBytes(txId), index);
