...
const EMPTY_ATTRIBUTES_CBOR = (0, util_1.HexBlob)('a0');
class BootstrapWitness {
    ...
    toCbor() {
        ...
        if (__classPrivateFieldGet(this, _BootstrapWitness_chainCode, "f").length / 2 !== 32)
            throw new util_1.InvalidStateError(`Chaincode must be 32 bytes long, but got ${__classPrivateFieldGet(this, _BootstrapWitness_chainCode, "f").length / 2} bytes long`);
        writer.writeStartArray(BOOTSTRAP_WITNESS_ARRAY_SIZE);
        writer.writeByteString((0, misc_1.hexToBytes)(__classPrivateFieldGet(this, _BootstrapWitness_vkey, "f")));
        writer.writeByteString((0, misc_1.hexToBytes)(__classPrivateFieldGet(this, _BootstrapWitness_signature, "f")));
        writer.writeByteString((0, misc_1.hexToBytes)(__classPrivateFieldGet(this, _BootstrapWitness_chainCode, "f")));
        writer.writeByteString((0, misc_1.hexToBytes)(__classPrivateFieldGet(this, _BootstrapWitness_attributes, "f")));
    ...
    static fromCbor(cbor) {
        ...
        if (length !== BOOTSTRAP_WITNESS_ARRAY_SIZE)
            throw new util_1.InvalidArgumentError('cbor', `Expected an array of ${BOOTSTRAP_WITNESS_ARRAY_SIZE} elements, but got an array of ${length} elements`);
        const vkey = util_1.HexBlob.fromBytes(reader.readByteString());
        const signature = util_1.HexBlob.fromBytes(reader.readByteString());
        const chainCode = util_1.HexBlob.fromBytes(reader.readByteString());
        const attributes = util_1.HexBlob.fromBytes(reader.readByteString());
        reader.readEndArray();
    ...
    toCore() {
        return {
            addressAttributes: util_1.Base64Blob.fromBytes((0, misc_1.hexToBytes)(__classPrivateFieldGet(this, _BootstrapWitness_attributes, "f"))),
    ...
    static fromCore(core) {
        if (!core.chainCode)
            throw new util_1.InvalidStateError('Chaincode must be present');
        if (core.chainCode.length / 2 !== 32)
            throw new util_1.InvalidStateError(`Chaincode must be 32 bytes long, but got ${core.chainCode.length / 2} bytes long`);
        return new BootstrapWitness(core.key, core.signature, core.chainCode, core.addressAttributes ? util_1.HexBlob.fromBase64(core.addressAttributes) : EMPTY_ATTRIBUTES_CBOR);
    }
    vkey() {
    ...
    setChainCode(chainCode) {
        if (chainCode.length / 2 !== 32)
            throw new util_1.InvalidStateError(`Chaincode must be 32 bytes long, but got ${chainCode.length / 2} bytes long`);
        __classPrivateFieldSet(this, _BootstrapWitness_chainCode, chainCode, "f");
        __classPrivateFieldSet(this, _BootstrapWitness_originalBytes, undefined, "f");
    }
    attributes() {
