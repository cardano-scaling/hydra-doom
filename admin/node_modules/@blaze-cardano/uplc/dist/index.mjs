...
import { HexBlob as HexBlob2 } from "@blaze-cardano/core";
...
import { HexBlob, PlutusData, fromHex as fromHex2, toHex } from "@blaze-cardano/core";
...
var _UPLCDecoder = class _UPLCDecoder extends Parser {
   ...
   */
  static fromHex(hex) {
    return new _UPLCDecoder(fromHex2(hex));
  }
  /**
...
// src/encoder.ts
import { PlutusData as PlutusData2, fromHex as fromHex3 } from "@blaze-cardano/core";
var UPLCEncoder = class extends Encoder {
  ...
  encodeData(type, data) {
    if (type === "Integer") {
      ...
      this.encodeByteString(fromHex3(PlutusData2.fromCore(data).toCbor()));
...
function stripCbor(cbor) {
  const cborReader = new CborReader(HexBlob2(cbor));
...
function applyParamsToScript(plutusScript, params, type) {
  ...
  return applyParams(HexBlob2(stripCbor(plutusScript)), ...paramsList);
}
function cborToScript(cbor, type) {
  if (type === "Native") {
    return Script.newNativeScript(NativeScript.fromCbor(HexBlob2(cbor)));
  } else {
    const cborReader = new CborReader(HexBlob2(cbor));
    while (cborReader.peekState() != CborReaderState.ByteString) {
      cborReader.readTag();
