...
__export(src_exports, {
  ...
  computeScriptData: () => computeScriptData,
  getScriptSize: () => getScriptSize,
  makeValue: () => makeValue
});
...
function negate(v) {
  ...
  return new import_core.Value(-v.coin(), tokenMap);
}
function sub(a, b) {
...
function positives(v) {
  ...
  return new import_core.Value(coin, tokenMap);
}
...
var zero = () => new import_core.Value(0n);
function makeValue(lovelace, ...assets2) {
  if (assets2.length == 0) {
    return import_core.Value.fromCore({ coins: lovelace });
  }
  const tokenMap = /* @__PURE__ */ new Map();
  for (const [asset, qty] of assets2) {
    ...
    tokenMap.set((0, import_core.AssetId)(asset), qty);
  }
  return import_core.Value.fromCore({
    coins: lovelace,
    assets: tokenMap
  });
}
...
function wideSelection(inputs, dearth) {
  ...
  let acc = new import_core2.Value(0n);
  for (let j = 0; j < 30; j++) {
    ...
    if (assetTypes(goal) <= 1) {
      ...
      let bestStep = [0, new import_core2.Value(0n), -1];
      ...
      if (bestStep[2] == -1) {
        throw new import_core2.UTxOSelectionError(
          "wide",
          dearth,
          availableInputs,
          selectedInputs,
...
function deepSelection(inputs, dearth) {
  ...
  for (let j = 0; j < 30; j++) {
    ...
    if (assetTypes(goal) == 0) {
      ...
      let bestStep = [0n, new import_core2.Value(0n), -1];
      for (let i = 0; i < availableInputs.length; i += 1) {
        const iValue = availableInputs[i].output().amount();
        let rating;
...
function micahsSelector(inputs, dearth) {
  ...
  if (!empty(finalDearth)) {
    throw new import_core2.UTxOSelectionError("final", finalDearth, inputs, selectedInputs);
  }
  return {
    selectedInputs,
    selectedValue: merge(
...
function getScriptSize(script) {
  const cborReader = new import_core3.CborReader(script.toCbor());
...
function computeScriptData(redeemers, datums, usedCostModels) {
  ...
  const writer = new import_core3.CborWriter();
  ...
  const scriptDataHash = (0, import_core3.blake2b_256)(hashedData);
  return {
...
var TxBuilder = class {
  ...
  constructor(params) {
    ...
    this.redeemers = import_core5.Redeemers.fromCore([]);
    ...
    this.params = params;
    this.body = new import_core5.TransactionBody(
      import_core5.CborSet.fromCore([], import_core5.TransactionInput.fromCore),
      [],
      0n,
      void 0
    );
  ...
  get burnAddress() {
    if (!this._burnAddress) {
      this._burnAddress = (0, import_core5.getBurnAddress)(this.networkId);
    }
  ...
  addReferenceInput(utxo) {
    const referenceInputs = this.body.referenceInputs() ?? import_core5.CborSet.fromCore([], import_core5.TransactionInput.fromCore);
  ...
  addInput(utxo, redeemer, unhashDatum) {
    ...
    for (const redeemer2 of redeemers) {
      if (redeemer2.tag() == import_core5.RedeemerTag.Spend && redeemer2.index() >= BigInt(insertIdx)) {
    ...
    if (redeemer !== void 0) {
      if (key.type == import_core5.CredentialType.KeyHash) {
        throw new Error(
      ...
      redeemers.push(
        import_core5.Redeemer.fromCore({
          index: insertIdx,
          purpose: import_core5.RedeemerPurpose["spend"],
      ...
      if (key.type == import_core5.CredentialType.ScriptHash) {
        this.requiredNativeScripts.add(key.hash);
      } else {
        this.requiredWitnesses.add((0, import_core5.HashAsPubKeyHex)(key.hash));
      }
    }
    const scriptRef = utxo.output().scriptRef();
    if (scriptRef) {
  ...
  addMint(policy, assets2, redeemer) {
    const insertIdx = this.insertSorted(
      this.consumedMintHashes,
      (0, import_core5.PolicyIdToHash)(policy)
    );
    const mint = this.body.mint() ?? /* @__PURE__ */ new Map();
    for (const asset of mint.keys()) {
      if (import_core5.AssetId.getPolicyId(asset) == policy) {
        throw new Error("addMint: Duplicate policy!");
      }
    }
    for (const [key, amount] of assets2.entries()) {
      mint.set(import_core5.AssetId.fromParts(policy, key), amount);
    }
    this.body.setMint(mint);
    if (redeemer) {
      this.requiredPlutusScripts.add((0, import_core5.PolicyIdToHash)(policy));
      const redeemers = [...this.redeemers.values()];
      for (const redeemer2 of redeemers) {
        if (redeemer2.tag() == import_core5.RedeemerTag.Mint && redeemer2.index() >= BigInt(insertIdx)) {
          redeemer2.setIndex(redeemer2.index() + 1n);
        }
      }
      redeemers.push(
        import_core5.Redeemer.fromCore({
          index: insertIdx,
          purpose: import_core5.RedeemerPurpose["mint"],
      ...
      this.requiredNativeScripts.add((0, import_core5.PolicyIdToHash)(policy));
    }
    return this;
  }
  /**
  ...
  checkAndAlterOutput(output) {
    {
      ...
      while (coin < minAda) {
        ...
        output = new import_core5.TransactionOutput(output.address(), amount);
        if (datum) {
  ...
  lockLovelace(address, lovelace, datum, scriptReference) {
    return this.lockAssets(
      address,
      new import_core5.Value(lovelace),
      datum,
      scriptReference
    );
  }
  ...
  deployScript(script, address = this.burnAddress) {
    const out = new import_core5.TransactionOutput(address, new import_core5.Value(0n));
  ...
  buildTransactionWitnessSet() {
    const tw = new import_core5.TransactionWitnessSet();
    const scriptLookup = {};
    ...
    for (const requiredScriptHash of this.requiredPlutusScripts) {
      ...
      if (lang == 1) {
        this.usedLanguages[import_core5.PlutusLanguageVersion.V1] = true;
      } else if (lang == 2) {
        this.usedLanguages[import_core5.PlutusLanguageVersion.V2] = true;
      } else if (lang == 3) {
        this.usedLanguages[import_core5.PlutusLanguageVersion.V3] = true;
      } else if (!lang) {
        throw new Error(
    ...
    if (sn.length != 0) {
      const cborSet = import_core5.CborSet.fromCore([], import_core5.NativeScript.fromCore);
      cborSet.setValues(sn);
      tw.setNativeScripts(cborSet);
    }
    if (s1.length != 0) {
      const cborSet = import_core5.CborSet.fromCore([], import_core5.PlutusV1Script.fromCore);
      cborSet.setValues(s1);
      tw.setPlutusV1Scripts(cborSet);
    }
    if (s2.length != 0) {
      const cborSet = import_core5.CborSet.fromCore([], import_core5.PlutusV2Script.fromCore);
      cborSet.setValues(s2);
      tw.setPlutusV2Scripts(cborSet);
    }
    if (s3.length != 0) {
      const cborSet = import_core5.CborSet.fromCore([], import_core5.PlutusV3Script.fromCore);
    ...
    const requiredWitnesses = Array.from(
      ...
      (_, i) => [
        (0, import_core5.Ed25519PublicKeyHex)(i.toString(16).padStart(64, "0")),
        (0, import_core5.Ed25519SignatureHex)(i.toString(16).padStart(128, "0"))
      ]
    );
    tw.setVkeys(import_core5.CborSet.fromCore(requiredWitnesses, import_core5.VkeyWitness.fromCore));
    tw.setRedeemers(this.redeemers);
    const plutusData = import_core5.CborSet.fromCore([], import_core5.PlutusData.fromCore);
  ...
  getPitch(spareAmount = 0n) {
    ...
    let inputValue = new import_core5.Value(withdrawalAmount);
    let outputValue = new import_core5.Value(bigintMax(this.fee, this.minimumFee));
    const mintValue = new import_core5.Value(0n, this.body.mint());
    ...
    for (const cert of this.body.certs()?.values() || []) {
      switch (cert.kind()) {
        case 0:
          outputValue = merge(
            outputValue,
            new import_core5.Value(BigInt(this.params.stakeKeyDeposit))
          );
          break;
        case 1:
          inputValue = merge(
        ...
        case 3:
          if (this.params.poolDeposit) {
            outputValue = merge(
              outputValue,
              new import_core5.Value(BigInt(this.params.poolDeposit))
            );
          }
          break;
        case 4:
    ...
    if (spareAmount != 0n) {
      return merge(tilt, new import_core5.Value(-spareAmount));
    }
   ...
   */
  getScriptData(tw) {
    const usedCostModels = new import_core5.Costmdls();
    for (let i = 0; i <= Object.keys(this.usedLanguages).length; i++) {
      if (this.usedLanguages[i]) {
        ...
        usedCostModels.insert(new import_core5.CostModel(i, cm));
      }
    }
  ...
  balanceChange(excessValue) {
    ...
    const output = new import_core5.TransactionOutput(this.changeAddress, excessValue);
  ...
  prepareCollateral() {
    ...
    if (providedCollateral.length > 0) {
      for (const utxo of providedCollateral) {
        ...
        if (coinAmount >= 5e6 && utxo.output().address().getProps().paymentPart?.type == import_core5.CredentialType.KeyHash) {
    ...
    if (!best) {
      for (const utxo of this.utxos.values()) {
        const coinAmount = this.getUtxoEffectiveCoin(utxo);
        if (coinAmount >= 5000000n && utxo.output().address().getProps().paymentPart?.type == import_core5.CredentialType.KeyHash) {
          const ranking = assetTypes(utxo.output().amount());
          if (ranking < rank) {
            rank = ranking;
            best = [utxo];
    ...
    const tis = import_core5.CborSet.fromCore([], import_core5.TransactionInput.fromCore);
    ...
    const ret = new import_core5.TransactionOutput(
  ...
  balanceMultiAssetChange(excessValue) {
    ...
    let output = new import_core5.TransactionOutput(this.changeAddress, zero());
    for (const [asset, qty] of Array.from(multiAsset.entries())) {
      ...
      if (newOutputValueByteLength > this.params.maxValueSize * 0.9) {
        ...
        output = new import_core5.TransactionOutput(
          this.changeAddress,
          makeValue(0n, [asset, qty])
        );
      } else {
        output = new import_core5.TransactionOutput(this.changeAddress, newOutputValue);
      }
    }
    this.addOutput(output);
    changeExcess = sub(changeExcess, output.amount());
  ...
  balanceCollateralChange() {
    ...
    this.body.setCollateralReturn(
      new import_core5.TransactionOutput(
        this.changeAddress,
        merge(collateralValue, new import_core5.Value(-totalCollateral))
      )
    );
  ...
  toCbor() {
    ...
    return new import_core5.Transaction(this.body, tw, this.auxiliaryData).toCbor();
  }
  /**
  ...
  async complete() {
    ...
    const inputs = [...this.body.inputs().values()];
    const preliminaryDraftTx = new import_core5.Transaction(
      this.body,
      new import_core5.TransactionWitnessSet()
    );
    ...
    this.balanceChange(excessValue);
    const draft_tx = new import_core5.Transaction(this.body, tw, this.auxiliaryData);
    let draft_size = draft_tx.toCbor().length / 2;
    this.calculateFees(draft_tx);
    excessValue = merge(
      excessValue,
      new import_core5.Value(
    ...
    if (this.redeemers.size() > 0) {
      ...
      if (this.fee > this.minimumFee) {
        if (this.fee - evaluationFee > this.minimumFee) {
          excessValue = merge(excessValue, new import_core5.Value(-evaluationFee));
          this.balanceChange(excessValue);
        } else {
          const feeChange = this.fee - this.minimumFee;
          excessValue = merge(excessValue, new import_core5.Value(-feeChange));
          this.balanceChange(excessValue);
        }
      }
    }
    ...
    do {
      ...
      this.balanceChange(import_core5.Value.fromCore(excessValue.toCore()));
    ...
    tw.setVkeys(import_core5.CborSet.fromCore([], import_core5.VkeyWitness.fromCore));
    return new import_core5.Transaction(this.body, tw, this.auxiliaryData);
  }
  /**
  ...
  addDelegation(delegator, poolId, redeemer) {
    const stakeDelegation = {
      __typename: import_core5.CertificateType.StakeDelegation,
      stakeCredential: delegator.toCore(),
      poolId
    };
    const delegationCertificate = import_core5.Certificate.newStakeDelegation(
      import_core5.StakeDelegation.fromCore(stakeDelegation)
    );
    const certs = this.body.certs() ?? import_core5.CborSet.fromCore([], import_core5.Certificate.fromCore);
    ...
    if (delegatorCredential.type == import_core5.CredentialType.ScriptHash) {
      if (redeemer) {
        ...
        redeemers.push(
          import_core5.Redeemer.fromCore({
            index: 256,
            // todo: fix
            purpose: import_core5.RedeemerPurpose["certificate"],
            data: redeemer.toCore(),
            executionUnits: {
              memory: this.params.maxExecutionUnitsPerTransaction.memory,
              steps: this.params.maxExecutionUnitsPerTransaction.steps
      ...
      this.requiredWitnesses.add((0, import_core5.HashAsPubKeyHex)(delegatorCredential.hash));
    }
    return this;
  }
  /**
  ...
  delegate(poolId, redeemer) {
    ...
    this.addDelegation(import_core5.Credential.fromCore(credential), poolId, redeemer);
    return this;
  }
  /**
  ...
  addRegisterStake(credential) {
    const stakeRegistration = new import_core5.StakeRegistration(
      credential.toCore()
    );
    const registrationCertificate = import_core5.Certificate.newStakeRegistration(stakeRegistration);
    const certs = this.body.certs() ?? import_core5.CborSet.fromCore([], import_core5.Certificate.fromCore);
  ...
  addWithdrawal(address, amount, redeemer) {
    const withdrawalHash = import_core5.Address.fromBech32(address).getProps().paymentPart?.hash;
    if (!withdrawalHash) {
      throw new Error(
    ...
    if (redeemer) {
      ...
      const redeemers = [...this.redeemers.values()];
      for (const redeemer2 of redeemers) {
        if (redeemer2.tag() == import_core5.RedeemerTag.Reward && redeemer2.index() >= BigInt(insertIdx)) {
          redeemer2.setIndex(redeemer2.index() + 1n);
        }
      }
      redeemers.push(
        import_core5.Redeemer.fromCore({
          index: insertIdx,
          purpose: import_core5.RedeemerPurpose["withdrawal"],
      ...
      const key = import_core5.Address.fromBech32(address).getProps().paymentPart;
      if (!key) {
        throw new Error(
          "addWithdrawal: The RewardAccount provided does not have an associated stake credential."
        );
  ...
  addRequiredSigner(signer) {
    const signers = this.body.requiredSigners() ?? import_core5.CborSet.fromCore([], import_core5.Hash.fromCore);
    this.requiredWitnesses.add(
      (0, import_core5.HashAsPubKeyHex)(import_core4.Hash28ByteBase16.fromEd25519KeyHashHex(signer))
    );
    const values = [...signers.values()];
    values.push(import_core5.Hash.fromCore(signer));
  ...
  getAuxiliaryDataHash(auxiliaryData) {
    return auxiliaryData ? (0, import_core5.blake2b_256)(auxiliaryData.toCbor()) : void 0;
  }
  /**
...
function assertPaymentsAddress(address) {
  ...
  if (props.paymentPart.type == import_core5.CredentialType.ScriptHash) {
    throw new Error(
...
function FromRecordRight(left, right) {
  const [Key, Value4] = [RecordKey(right), RecordValue(right)];
  return type_exports.IsLiteralString(left) && type_exports.IsNumber(Key) && IntoBooleanResult(Visit3(left, Value4)) === ExtendsResult.True ? ExtendsResult.True : type_exports.IsUint8Array(left) && type_exports.IsNumber(Key) ? Visit3(left, Value4) : type_exports.IsString(left) && type_exports.IsNumber(Key) ? Visit3(left, Value4) : type_exports.IsArray(left) && type_exports.IsNumber(Key) ? Visit3(left, Value4) : type_exports.IsObject(left) ? (() => {
    for (const key of Object.getOwnPropertyNames(left.properties)) {
      if (Property(Value4, left.properties[key]) === ExtendsResult.False) {
        return ExtendsResult.False;
      }
    }
    return ExtendsResult.True;
...
var Data = {
  ...
  void: () => import_core6.PlutusData.newConstrPlutusData(new import_core6.ConstrPlutusData(0n, new import_core6.PlutusList())),
  castFrom,
  castTo,
  to,
  from
...
function to(data, type, recType) {
  if (type === void 0 || type.description == "Any Data.") {
    if (typeof data == "bigint") {
      return import_core6.PlutusData.newInteger(data);
    } else if (typeof data === "string") {
      return import_core6.PlutusData.newBytes((0, import_core6.fromHex)(data));
    } else if (typeof data === "boolean") {
      return import_core6.PlutusData.newConstrPlutusData(
        new import_core6.ConstrPlutusData(BigInt(data ? 1 : 0), new import_core6.PlutusList())
      );
    } else if (typeof data == "object") {
      if (data instanceof Array) {
        const list = new import_core6.PlutusList();
        for (let i = 0; i < data.length; i++) {
          list.add(to(data[i]));
        }
        return import_core6.PlutusData.newList(list);
      } else if (data instanceof Map) {
        const plutusMap = new import_core6.PlutusMap();
        for (const [key, value] of data.entries()) {
          plutusMap.insert(to(key), to(value));
        }
        return import_core6.PlutusData.newMap(plutusMap);
      } else if (data instanceof Constr) {
        return import_core6.PlutusData.newConstrPlutusData(
          new import_core6.ConstrPlutusData(BigInt(data.index), to(data.fields).asList())
        );
      }
    }
  }
...
function from(data, type) {
  if (type == void 0 || type != null && typeof type == "object" && "description" in type && type.description == "Any Data.") {
    ...
    if (kind == import_core6.PlutusDataKind.Integer) {
      return data.asInteger();
    } else if (kind == import_core6.PlutusDataKind.Bytes) {
      return (0, import_core6.toHex)(data.asBoundedBytes());
    } else if (kind == import_core6.PlutusDataKind.ConstrPlutusData) {
      const constr = data.asConstrPlutusData();
      const list = from(import_core6.PlutusData.newList(constr.getData()));
      return new Constr(
        Number(constr.getAlternative()),
        list
      );
    } else if (kind == import_core6.PlutusDataKind.List) {
      ...
      return returnList;
    } else if (kind == import_core6.PlutusDataKind.Map) {
...
function castTo(struct, schemaType, recType, recShape) {
  ...
  if (struct instanceof import_core6.PlutusData) {
    return struct;
  }
  if ("anyOf" in schema) {
    ...
    if (isBoolean(schemaEnum)) {
      ...
      return import_core6.PlutusData.newConstrPlutusData(
        new import_core6.ConstrPlutusData(
          struct ? BigInt(trueIdx) : BigInt(falseIdx),
          new import_core6.PlutusList()
        )
      );
      ...
      if (struct === null) {
        return import_core6.PlutusData.newConstrPlutusData(
          new import_core6.ConstrPlutusData(BigInt(noneIdx), new import_core6.PlutusList())
        );
      } else {
        ...
        const someList = new import_core6.PlutusList();
        someList.add(castTo(struct, fields[someIdx], recType, recShape));
        return import_core6.PlutusData.newConstrPlutusData(
          new import_core6.ConstrPlutusData(BigInt(someIdx), someList)
        );
      }
    }
    switch (typeof struct) {
      case "string": {
        ...
        return import_core6.PlutusData.newConstrPlutusData(
          new import_core6.ConstrPlutusData(BigInt(enumIndex), new import_core6.PlutusList())
        );
      }
      case "object": {
        ...
        const plutusList = new import_core6.PlutusList();
        fields.forEach((f) => plutusList.add(f));
        return import_core6.PlutusData.newConstrPlutusData(
          new import_core6.ConstrPlutusData(BigInt(enumEntry["index"]), plutusList)
        );
      }
    }
  }
  ...
  switch (schema["dataType"]) {
    case "integer": {
      ...
      return import_core6.PlutusData.newInteger(struct);
    }
    case "bytes": {
      if (typeof struct !== "string") {
        throw new Error("Could not type cast to bytes.");
      }
      return import_core6.PlutusData.newBytes((0, import_core6.fromHex)(struct));
    }
    case "constructor": {
      if (schema["fields"].length == 0 && typeof struct == "string") {
        ...
        return import_core6.PlutusData.newConstrPlutusData(
          new import_core6.ConstrPlutusData(BigInt(schema["index"]), new import_core6.PlutusList())
        );
      }
      if (isVoidConstructor(schema)) {
        ...
        return import_core6.PlutusData.newConstrPlutusData(
          new import_core6.ConstrPlutusData(0n, new import_core6.PlutusList())
        );
      ...
      const fields = new import_core6.PlutusList();
      ...
      return import_core6.PlutusData.newConstrPlutusData(
        new import_core6.ConstrPlutusData(BigInt(schema["index"]), fields)
      );
    }
    case "list": {
      ...
      if (schema["items"] instanceof Array) {
        ...
        return import_core6.PlutusData.newList(fields);
      } else {
    ...
    case "map": {
      ...
      const map = new import_core6.PlutusMap();
      ...
      return import_core6.PlutusData.newMap(map);
    }
  }
  throw new Error("Could not type cast struct.");
}
function castFrom(rawData, schema) {
  ...
  if ("anyOf" in schema) {
    ...
    if (!(kind == import_core6.PlutusDataKind.ConstrPlutusData)) {
  ...
  switch (schema["dataType"]) {
    ...
    case "bytes": {
      if (kind !== import_core6.PlutusDataKind.Bytes) {
        throw new Error("Could not type cast to integer.");
      }
      return (0, import_core6.toHex)(rawData.asBoundedBytes());
    }
    case "constructor": {
      if (kind !== import_core6.PlutusDataKind.ConstrPlutusData) {
        throw new Error("Could not type cast to constructor.");
    ...
    case "list": {
      if (kind !== import_core6.PlutusDataKind.List) {
    ...
    case "map": {
      if (!(kind == import_core6.PlutusDataKind.Map)) {
        throw new Error("Could not type cast to map.");
      }
